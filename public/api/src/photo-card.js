...
export class PhotoCard extends LitElement {
  static properties = { photo: { type: Object }, reaction: { type: String } };
  static styles = css`
    :host { display:block }
    ...
    return html`
      <div class="card">
        ...
        <div class="meta">
          ...
          <div class="actions">
            <button aria-pressed=${this.reaction==='like'} @click=${() => this.dispatchEvent(new CustomEvent('react', { detail:{ id:p.id, value:'like' } }))}>👍</button>
            <button aria-pressed=${this.reaction==='dislike'} @click=${() => this.dispatchEvent(new CustomEvent('react', { detail:{ id:p.id, value:'dislike' } }))}>👎</button>
            <button @click=${() => this.dispatchEvent(new CustomEvent('share', { detail:{ photo:p } }))}>↗</button>
          </div>
        </div>
      </div>
    `;
